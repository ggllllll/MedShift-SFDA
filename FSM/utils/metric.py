...
def test(model, dataloader, args, adabn_loader=False):
    ...
    with torch.no_grad():
        ...
        for i, data in bar:
            ...
            if args.is_contrast:
                ...
                output = model(img)
            case_out = output.cpu().numpy()
            case_seg = np.zeros((512, 512))
            case_seg[case_out[0][0] > 0.5] = 255
            case_seg[case_out[0][1] > 0.5] = 128
            case_seg_f = Image.fromarray(case_seg.astype(np.uint8)).resize((512, 512), resample=Image.NEAREST)
            case_seg_f.save(os.path.join(args.snapshot_dir, data[1][0].split('/')[-1]))
            # if args.save_result:
    ...
    # print()
    eval_print_all_CU(args.snapshot_dir, args.test_mask_dir)
    return 1
def test_epoch(model, dataloader, epoch, args, adabn_loader=False):
    metrics = Metrics(['recall', 'specificity', 'Dice',
                       'ACC_overall', 'IoU_poly', 'IoU_bg', 'IoU_mean'])
    snapshot_dir_epoch = os.path.join(args.snapshot_dir, epoch)
    os.mkdir(snapshot_dir_epoch)
    if args.is_adabn:
        # for m in model.modules():
        #     classname = m.__class__.__name__
        #     if classname.find('BatchNorm') != -1:
    ...
    with torch.no_grad():
        ...
        for i, data in bar:
            ...
            case_seg[case_out[0][1] > 0.5] = 128
            case_seg_f = Image.fromarray(case_seg.astype(np.uint8)).resize((512, 512), resample=Image.NEAREST)
            case_seg_f.save(os.path.join(snapshot_dir_epoch, data[1][0].split('/')[-1]))
            # if args.save_result:
            #     prediction = (output >= 0.5).float()
            #     prediction = tensor_to_PIL(prediction)
            #     name = data[1][0].split('/')[-1]
    ...
    #          metrics_result['IoU_poly'], metrics_result['IoU_bg'], metrics_result['IoU_mean']))
    # print()
    eval_print_all_CU(snapshot_dir_epoch, args.test_mask_dir)
    return 1
