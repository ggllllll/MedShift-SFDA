...
class UNet(nn.Module):
    def __init__(self, resnet='resnet34', num_classes=2, pretrained=False, inversion=False, contrast=False):
        super().__init__()
        cut, lr_cut = [8, 6]
    ...
    def forward(self, x, rfeat=False):
        x = F.relu(self.rn(x))
        x5 = x
        ...
        output = self.up5(x)
        if self.inversion:
            return self.sfs[1].features, self.sfs[2].features, self.sfs[3].features
        if self.contrast:
            return x5, torch.sigmoid(output) # changed!
        if not rfeat:
            return output
        else:
            return output, fea
