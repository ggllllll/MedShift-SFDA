...
from utils.metric import Metrics, setup_seed, test, test_epoch
...
def get_arguments():
    ...
    parser.add_argument("--test-mask-dir", type=str, default=test_target_mask_DIR,
                        help="test_target_mask_DIR.")
...
def main():
    ...
    model = UNet(contrast=args.is_contrast).cuda()
    # model = Deeplab(1, pretrained=False, contrast=args.is_contrast).cuda()
    model.load_state_dict(torch.load(args.restore_from)['model_state_dict'])
    ...
    for epoch in range(args.num_steps):
        ...
        for i_iter, batch in enumerate(contrast_loader_target):
            ...
            loss_pseudo = source_criterion(source_output[:, 0], (label[:, 0] > 0) * 1.0, weight=weight) + source_criterion(target_output[:, 0], (label[:, 0] > 0) * 1.0, weight=weight) + source_criterion(source_output_rotate[:, 0], (label_rotate[:, 0] > 0) * 1.0, weight=weight) + source_criterion(target_output_rotate[:, 0], (label_rotate[:, 0] > 0) * 1.0, weight=weight) + source_criterion(source_output[:, 1], (label[:, 0] == 2) * 1.0, weight=weight) + source_criterion(target_output[:, 1], (label[:, 0] == 2) * 1.0, weight=weight) + source_criterion(source_output_rotate[:, 1], (label_rotate[:, 0] == 2) * 1.0, weight=weight) + source_criterion(target_output_rotate[:, 1], (label_rotate[:, 0] == 2) * 1.0, weight=weight)
    ...
    # begin test on target domain
        dice = test_epoch(model, test_loader_target, str(epoch), args)
    # dice = test(model, test_loader_target, args)
        torch.save(model.state_dict(), os.path.join(os.path.join(args.snapshot_dir, str(epoch)), 'FSM_last.pth'))

    # torch.save(model.state_dict(), os.path.join(SNAPSHOT_DIR, 'FSM_last.pth'))
