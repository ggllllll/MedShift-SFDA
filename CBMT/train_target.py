...
def main(args):
    ...
    dataset_train = fundus_dataloader.FundusSegmentation_2transform(base_dir=root_folder, dataset=args.dataset,
                                                                    split='train/ROIs',
                                                                    img_list=tr_img_list,
                                                                    label_list=tr_label_list,
                                                                    transform_weak=composed_transforms_test,
                                                                    transform_strong=composed_transforms_train)
    dataset_train_weak = fundus_dataloader.FundusSegmentation(base_dir=root_folder, dataset=args.dataset,
                                                              split='train/ROIs',
                                                              img_list=tr_img_list,
                                                              label_list=tr_label_list,
                                                              transform=composed_transforms_test)
    dataset_test = fundus_dataloader.FundusSegmentation(base_dir=root_folder, dataset=args.dataset, split='test/ROIs',
                                                        img_list=ts_img_list,
                                                        label_list=ts_label_list,
                                         transform=composed_transforms_test)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tr_csv', help='training csv file.', default=[
        r' '])
    parser.add_argument('--ts_csv', help='test csv file.', default=[r' '])

    args = parser.parse_args()
    for dataset in [' ']:
        args.dataset = dataset
        args.tr_csv = [r' '.replace(' ', dataset)]
        args.ts_csv = [r' '.replace(' ', dataset)]
        main(args)
