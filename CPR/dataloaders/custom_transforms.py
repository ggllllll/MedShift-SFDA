...
class add_salt_pepper_noise_sim():
    def __call__(self, sample):
...
class Resize1(object):
    def __init__(self, size1, size2):
        self.size_img = size1
        self.size_label = size2
        self.size_gt = size2

    def __call__(self, sample):
        img = sample['image']
        mask = sample['pseudo_label']
        name = sample['img_name']
        gt = sample['gt']
        if self.size_img != None:
            img = img.resize((self.size_img, self.size_img), Image.BILINEAR)
        #mask = mask.resize((self.size_label, self.size_label), Image.NEAREST)
        if self.size_label != None:
            mask = F.interpolate(mask.unsqueeze(0), size=(self.size_label,self.size_label), mode='nearest')

            mask = mask.squeeze()
    
        if self.size_gt != None:
            gt = gt.resize((self.size_gt, self.size_gt), Image.NEAREST)

        sample = {'image': img, 'pseudo_label': mask, 'img_name': name, 'gt': gt}
        
        return sample

class Resize2(object):
    def __init__(self, size1=None,size2=56,size3=None,size4=None):
        self.size_img = size1
        self.size_label = size2
        self.size_prob = size3
        self.size_gt = size4
    ...
    def __call__(self, sample):
        ...
        if self.size_prob != None:
            prob = F.interpolate(prob.unsqueeze(0), size=(self.size_prob,self.size_prob), mode='nearest')###'bilinear'

            prob = prob.squeeze()
        if self.size_gt != None:
            gt = gt.resize((self.size_gt, self.size_gt), Image.NEAREST)

        sample = {'image': img, 'pseudo_label': mask, 'img_name': name, 'prob':prob, 'gt':gt}
...
class ToTensor1(object):
    ...
    def __call__(self, sample):
        ...
        map = np.array(sample['gt']).astype(np.uint8).transpose((2, 0, 1))
