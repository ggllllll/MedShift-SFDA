...
if __name__ == '__main__':
    ...
    parser.add_argument("--weights", type=str, default='./log/sim_learn_.pth.tar')
    ...
    parser.add_argument('--pseudo', type=str, default=r'')
    parser.add_argument('--radius',type=int,default=4)

    args = parser.parse_args()
    radius = args.radius

    # model = netd.DeepLab(num_classes=2, backbone='mobilenet', output_stride=args.out_stride, sync_bn=args.sync_bn, freeze_bn=args.freeze_bn, image_res=512, radius=radius)
    from models.unet import UNet
    model = UNet(image_res=512, radius=radius)
    ...
    for iter, (img, _, name, prob, gt) in enumerate(infer_data_loader):
        ...
        dheight = int(np.ceil(32))
        dwidth = int(np.ceil(32))
    ...
    # if args.dataset=="MESSIDOR_Base1":
    #     np.savez('./log/REFUGE/pseudolabel_MESSIDOR_Base1_new', pseudo_label_dic, prob_dic)
    #
    # if args.dataset=="MESSIDOR_Base2":
    #     np.savez('./log/REFUGE/pseudolabel_MESSIDOR_Base2_new', pseudo_label_dic, prob_dic)
    #
    # if args.dataset=="MESSIDOR_Base3":
    #     np.savez('./log/REFUGE/pseudolabel_MESSIDOR_Base3_new', pseudo_label_dic, prob_dic)
    # if args.dataset=="DS":
    #     np.savez('./log/pseudolabel_DS_new', pseudo_label_dic, prob_dic)
    if args.dataset=="":
        np.savez('', pseudo_label_dic, prob_dic)
    # if args.dataset=="REFUGE-val":
    #     np.savez('./log/pseudolabel_REFUGE_new', pseudo_label_dic, prob_dic)