...
from plot.file_management import get_filenames, save_csv_mean_segmentation_performance, save_csv_segmentation_table
...
def dice_coefficient_1(binary_segmentation, binary_gt_label, binary_segmentation_not, binary_gt_label_not):
    ...
    Pre = intersection / (segmentation_pixels+float("1e-8"))
...
def evaluate_binary_segmentation_1(segmentation, gt_label):
    ...
    cup_dice, cup_jac, cup_acc, cup_sen, cup_pre = dice_coefficient_1(segmentation == 0, gt_label == 0,
                                                                      segmentation > 0, gt_label > 0)
    # compute the Dice coefficient for the optic disc
    disc_dice, disc_jac, disc_acc, disc_sen, disc_pre = dice_coefficient_1(segmentation < 255, gt_label < 255,
                                                                           segmentation == 255, gt_label == 255)
...
def evaluate_binary_segmentation_CDR_ROC(segmentation, gt_label):
    '''
    Compute the evaluation metrics of the REFUGE challenge by comparing the segmentation with the ground truth
    Input:
        segmentation: binary 2D numpy array representing the segmentation, with 0: optic cup, 128: optic disc, 255: elsewhere.
    ...
    return cup_dice, cup_jac, cup_acc, cup_sen, cup_pre, disc_dice, disc_jac, disc_acc, disc_sen, disc_pre, cdr, vertical_cup_to_disc_ratio(segmentation)
...
def evaluate_segmentation_results(segmentation_folder, gt_folder, output_path=None, export_table=False,
    ...
    image_filenames = get_filenames(segmentation_folder, 'png')
    if len(image_filenames) == 0:
        print(
            '** The segmentation folder does not include any png file. Check the files extension and resubmit your results.')
        raise ValueError()
